...
export default function FloatingSocial() {
  ...
  const socialLinks = [
    ...
    },
    {
      ...
      bgClass: 'bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] hover:opacity-90',
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 md:w-5 md:h-5">
  ...
  return (
    ...
    >
        ...
        {socialLinks.map((link) => (
          <div key={link.name} className="group relative flex items-center">
            <a
              ...
              className={`w-8 h-8 md:w-10 md:h-10 hover:w-12 hover:pl-2 ${link.bgClass}`}
              aria-label={link.name}
            >
              {link.icon}
            </a>
            {!isMobile && (
              <span className="absolute left-full ml-2 bg-[#1a1a1a] text-white text-xs font-bold px-3 py-1.5 rounded shadow-lg opacity-0 pointer-events-none -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                {link.name}
              </span>
            )}
          </div>
      ...
      {/* Collapse/Expand tab for Mobile */}
      {isMobile && (
        <div
